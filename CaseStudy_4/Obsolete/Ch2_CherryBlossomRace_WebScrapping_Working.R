...
for (p in years){
#  ageList[[paste0('age',p)]] <- assign(paste0('age',p),(cbMenSub[ cbMenSub$year == p, "age" ]))}
...
pdf("./Figures/Testing_Fig_2.13_CB_AgeDensity99vs12.pdf", width = 8, height = 6)
oldPar = par(mar = c(4.1, 4.1, 1, 1))
lc = 1
...
for (j in years){
    if (j != 1999){
      lc = lc + 1 # Loop count
      lines(density(vx[[as.character(paste0('age',j))]], na.rm = TRUE), 
...
brewer.pal(n=12, name='Paired')
 co <- palette(c('black','grey', brewer.pal(n=12, name='Paired')))
palette()
co[2] <- c()
...
Final <- b + geom_jitter(aes(color=runTime),position=position_jitter(width=.4, height=.5),cex=.9) + 
...
Final$labels$colour = "Run Time(min.)"
Final
#-------
